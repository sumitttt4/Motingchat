    ...
    <section ref={revealRef as React.RefObject<HTMLElement>} id="features" className="py-24 px-6">
      <div className="max-w-content mx-auto">
        ...
        <div className="flex items-center justify-between mb-12">
          ...
          <span className="text-[11px] font-medium text-gray-400 uppercase tracking-wider">02 / 08</span>
        </div>

        <h2 className="text-[40px] md:text-[48px] font-bold text-heading leading-[1.1] tracking-tight mb-16">
          Not just transcription.<br />
          <span className="text-body font-normal text-[32px]">Complete meeting intelligence.</span>
        </h2>

        <div className="grid md:grid-cols-[1fr,1.2fr] gap-12 items-start">
          {/* Left — Feature illustration area */}
          <div className="bg-card rounded-2xl border border-border-subtle p-8 min-h-[400px] flex items-center justify-center sticky top-20 overflow-hidden">
            <div key={openIdx} className="text-center animate-scale-in">
              <div className="w-16 h-16 bg-accent/10 rounded-2xl flex items-center justify-center mx-auto mb-4 text-accent group-hover:scale-110 transition-transform">
                {features[openIdx].icon}
              </div>
              <p className="text-[22px] font-bold text-heading mb-2">{features[openIdx].title}</p>
              <p className="text-[14px] text-body max-w-[300px] mx-auto leading-relaxed">{features[openIdx].desc}</p>
            </div>
          </div>
            ...
            {features.map((feat, i) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform duration-300 ${openIdx === i ? 'rotate-180 text-accent' : ''}`} />
                </div>
